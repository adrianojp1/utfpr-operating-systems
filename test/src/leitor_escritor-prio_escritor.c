...
sem_t s_leitor_lock, s_escritor_lock;
ls_t ls_escritores, ls_leitores;
...
void *leitor_body(void *t_args) {
    ...
    while (1) {
        sem_wait(&s_leitor_lock);
        enter(&ls_leitores);
        sem_post(&s_leitor_lock);
...
void execute() {
    ...
    init_lightswitch(&ls_escritores, &s_leitor_lock);
